function Avet(canvasId, imgspath, options, scorechange, gameendcalback, timedowncalback) {
    var _this = this;
    ...
    var mask;
    var hands, line, handstween, linetween, tishimask; 
 //Hands and lines and corresponding animations

    var touching = false;   
//The pan that must be started by panstart is valid

    var canxiao = null;

    function preparebackground() {  //drawing background
    ...
    function loadimg() {    //loading image
    ...
    function randomrect() {     //Initialization data
        for (var i = 0; i < hi; i++) {
            for (var j = 0; j < wi; j++) {
                var notin = {};
                ...
                while (true) {
                    ...
                    if (!notin[b]) {
                        maps[i][j] = {
                            b: null,    //bitimage
                            c: b,       //type
                            x: j,       //horizontal position
                            y: i,       //vertical position
                            ax: 0,      //horizontal traversal plus one longest common subsequence algorithm
                            ay: 0,      //vertical traversal plus one longest common subsequence algorithm
                        };
                        break;
                    }
                }
    ...
    function prepareScene() {   //Initialization scenario
        for (var i = 0; i < hi; i++) {
            maps[i] = new Array(wi);
        }
    ...
    function addEvent() {   //Add event
        ...
        function panendfun() {//called when moving an element
                ...
                for (var i = hi - 1; i >= 0; i--) {
                    if (maps[i][j] == null) {
                        ...
                        if (dk !== null) {
                            ...
                            maps[i][j] = {
                                b: null,    //bitimage
                                c: Math.floor(Math.random() * classnum),       //Types of
                                x: j,       //horizontal position
                                y: i,       //Vertical position
                                ax: 0,      //Horizontal traversal plus one longest common subsequence algorithm
                                ay: 0,      //Vertical traversal plus one longest common subsequence algorithm
    ...
    function placeagain(isstart) { //Reshuffle after dead chess

        var boxsquare = wi * hi;
        for (var k = boxsquare - 1; k >= 0; k--) {   //Revision of the classic shuffling algorithm, plus single card re-sampling
    ...
    function panduan(isstart) {  //Determine whether it is dead chess
    ...
    function isdie(y, x, getposition) {  //Determine the dead chess at a certain point
    ...
    function tishi() {  //Display prompt gesture
    ...
    this.start = function () {
        if (gaming) {
            ...
            addScore(0);
            canmove = true;
            gaming = true;  //Set the logo of the game in progress
            gameend = false;    //Set the logo of the end of the game
